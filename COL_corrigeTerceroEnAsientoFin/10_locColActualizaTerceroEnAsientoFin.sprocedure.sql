...
create PROCEDURE [DBO].[locColActualizaTerceroEnAsientoFin]
AS
...
--select fi.*, p.custname, gl.ormstrid, gl.ormstrnm
from gl20000 gl
...
where gl.sourcdoc = 'DG'
and datediff(day, '1/1/17', gl.trxdate) >= 0
and fi.custvndr != ''
and gl.ormstrid != fi.custvndr
;

--Actualiza terceros en caso de transacciones bancarias
...
LEFT JOIN rm00101 rm
	...
	and fi.terctype = 1
where gl.sourcdoc in ( 'CMTRX', 'CMXFR')
and gl.ormstrid != fi.custvndr
and datediff(day, '1/1/17', gl.trxdate) >= 0
and fi.custvndr != '';
GO
